...
<Update label="June 16, 2026" description="Weekly release">

Quality and connectivity improvements across chat, Genie Cloud, and workspace integrations.

## New features

- **Connect external AI clients via the Genie MCP server.** Use Claude Desktop, Cursor, or any MCP-compatible AI client to manage your Genie workspaces and projects programmatically — create projects, send follow-up prompts, check build status, and chat with Genie AI without opening the web app. See [Genie MCP server](/workspaces/mcp-server).
- **"Continue with Genie AI" after chat answers.** When Genie answers a question in chat mode, a new **Continue with Genie AI** button appears under the response. It opens the full workspace chat agent and pre-fills it with your conversation context, so you can seamlessly move from a quick Q&A to a full building session.

## Updates

- **Genie Cloud settings apply automatically.** Onboarding and private mode settings now sync to your live Genie Cloud instance the moment you save them — no need to click Re-run bootstrap afterwards.
- **Softer notification sound.** The chime that plays when a build or chat response finishes has been updated to a gentler two-note ascending tone, replacing the previous sharp single-note alert.
