...
This week brings the Genie MCP API, an expanded workspace chat, and a smarter project Brain.

## New features

- **Genie MCP API.** Connect Claude Desktop, Cursor, or any MCP-compatible client to your workspace and manage projects through natural language — create projects, send follow-up prompts, preview deployments, manage members, and search past chats. See [Genie MCP API](/workspaces/mcp-api).
- **Workspace Chat interactive controls.** The workspace chat AI can now render radio-group selects, multi-line text areas, and checkbox lists inline in the conversation — making it easier to make choices without typing free-form replies. See [Workspace Chat](/workspaces/chat).
- **"What's next" in the project Brain.** The Brain now surfaces a ranked panel of predicted next prompts — features, changes, or fixes the AI thinks you are most likely to want — based on your project's build history and goals. Open a project, go to **Brain**, and look for the **What's next** section.

## Updates

- **Softer notification sound.** The chime that plays when a build finishes has been replaced with a gentler two-note ascending tone, making it less jarring when working in a quiet environment.
