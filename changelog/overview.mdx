...
<Update label="September 25, 2026" description="Weekly release">

Covers what shipped September 24, plus a knowledge-access feature from a day earlier that the docs missed. Genie AI can now build a structured model of your knowledge base — entities and the relationships between them — and admins can fence a folder off to just the people who should see it. Genie Tools tables and uploads both get more capable, and the Genie MCP Server catches up on workspace administration.

## Updates

- **Genie AI builds a structured model of your knowledge base.** Turn on automatic reading (owners and admins, from the Knowledge page) and Genie reads every document to find the entities in it — people, organisations, decisions, terms, and more — and the relationships between them, each traceable back to the passage it came from. Browse the result in the new **Classes and relations** panel on the Knowledge page, or ask Genie AI to review what it found, merge duplicate entities, or re-read everything from scratch. See [Knowledge base](/workspaces/overview#knowledge-model).
- **Restrict a knowledge folder to specific people or roles.** Owners and admins can fence off a folder — an "access area" — so only chosen roles, groups, or individual members can read or add to what's inside it; everyone else doesn't see that it exists. Manage areas from workspace settings → **Knowledge access**. See [Knowledge base](/workspaces/overview#access-areas).
- **Genie Tools tables and uploads get more capable.** Every table now has search, sortable columns, and sticky headers, and a tool can turn on selectable rows for bulk actions like exporting just what you checked. Uploads — into an Import block, a form's file field, or your knowledge base — now take files up to 200 MB, with large ones compressed automatically, and a cell holding a single link renders as a clickable thumbnail or download link instead of raw text. See [Genie Tools](/workspaces/genie-tools).
- **Genie MCP Server catches up on workspace administration.** External AI clients connected via the Genie MCP Server can now manage custom roles and permissions, create and manage workspace API keys, and set up knowledge access areas — plus check your own per-integration connection status and a knowledge document's cover picture. See [Genie MCP Server](/workspaces/mcp-api).

## Fixes

- **Fixed Genie AI sometimes replying with only a tool result or card and no words alongside it.** It now always gives a conversational answer alongside whatever it did.

</Update>

<Update label="September 24, 2026" description="Weekly release">

Covers what shipped September 23. Genie Agents can branch: a new If/else step tests earlier results with AND, OR, or NOT, and any step can be set to run only on the branch that follows. The knowledge base also reaches OneDrive and SharePoint as a source alongside Google Drive, and every document now carries the date it's about, not just the day it was uploaded.

## Updates

- **Genie Agents can branch.** A new If/else step tests one or more rules against earlier results — combined with AND, OR, or NOT — and any step can be set to run only if that branch's answer was yes or no. See [Branching](/workspaces/workflow-apps#branching).
- **OneDrive and SharePoint join Google Drive as a knowledge base source.** Browse your OneDrive or any SharePoint site you follow, import a file directly, or keep a whole library or folder in sync the same way you already can with a Drive folder. See [Cloud drives](/workspaces/overview#cloud-drives).
- **Synced folders now hold onto a document when its file moves or disappears**, instead of losing track of it: move a file between two folders you sync and it stays the same knowledge document, and remove one from its source and the document sticks around — marked **Removed from the drive** — for 7 days in case it comes back, instead of being deleted right away. See [Cloud drives](/workspaces/overview#cloud-drives).
- **Every knowledge document now carries the date it's about**, separate from its upload day — worked out automatically from the file name, title, or a connected drive's last-modified time, and correctable from the document's Rename or move dialog or by asking Genie AI. See [Knowledge base](/workspaces/overview#knowledge-base).
- **A simple Genie Agent (no branching) now draws as a clean top-to-bottom stack on the chat canvas**, instead of the sideways grid the editor uses. See [Open a Genie Tool or Agent beside the chat](/workspaces/overview#open-a-genie-tool-or-agent-beside-the-chat).

## Fixes

- **Fixed accepting a workspace invite when every seat is taken showing a generic error page** — it now shows a clear "workspace is full" message, and a single-use invite is no longer burned on a failed acceptance.
- **Fixed the agent canvas beside the chat not following a later edit** — renaming a Genie Agent or changing its steps in a later message now updates the docked canvas, instead of leaving it showing the agent's original version.

</Update>

<Update label="September 23, 2026" description="Weekly release">

Covers what shipped September 22. The knowledge base uploader catches up to what people actually have on hand: PDFs — including scanned ones, read automatically with OCR — can now be uploaded directly, and a document can be found by its filename alone, not just what's written inside it. Voice dictation also reaches Genie Tools' own forms.

## Updates

- **Upload PDFs directly to your knowledge base.** No more routing them through chat first — drop a PDF into the uploader like any other document. See [Knowledge base](/workspaces/overview#knowledge-base).
- **A scanned PDF with no text layer is now read automatically with OCR** instead of being rejected, so a folder of scans uploads the same way a folder of ordinary documents does. See [Knowledge base](/workspaces/overview#knowledge-base).
- **Genie AI can now find a knowledge document by its filename or description**, not only by what's written inside it — useful for a spreadsheet of figures or a scanned contract with little body text to search. See [Knowledge base](/workspaces/overview#knowledge-base).
- **Voice dictation now works in a Genie Tool's own form fields**, the same as it already does in chat. See [Talk instead of type](/workspaces/overview#talk-instead-of-type).

## Fixes

- **Fixed onboarding sometimes failing to finish if a new build landed mid-flow** — "Try again" could loop forever instead of completing.
- **Fixed the "Connect [integration]?" chat card showing in English inside an otherwise German conversation.**
- **Fixed a Genie Tool sometimes showing raw data instead of a formatted table**, and an action no longer needlessly re-runs another block's AI step when it doesn't need to. See [Genie Tools](/workspaces/genie-tools).
- **Genie Tools no longer shows internal error detail (like a node name or file path) to workspace members when a block fails** — just a plain message, with the detail still available to admins. See [Genie Tools](/workspaces/genie-tools).
- **Genie Agents: a step that produces no output is now correctly reported as a failed step**, instead of a silent success with a blank result. See [Genie Agents](/workspaces/workflow-apps).

</Update>

<Update label="September 22, 2026" description="Weekly release">

Covers what shipped September 21. Genie Tools become true multi-page apps — including a one-question-at-a-time form mode — and can now import files straight from your knowledge base. A Genie Tool or Genie Agent can also open in a canvas beside the chat instead of scrolling past in the transcript, the command palette gets a full visual redesign, and workspace settings now pick a whole color palette instead of a single accent.

## Updates

- **Genie Tools now lay out as multi-page apps.** A tool with a lot going on splits itself into focused pages — typically Dashboard, Upload, Manage, and Analytics — instead of one long scroll, and a page can also run as a **form**: one question at a time, with a progress line and a thank-you screen at the end. See [Genie Tools](/workspaces/genie-tools).
- **A Genie Tool's import block and file fields can now pull straight from your knowledge base**, not just from disk, and a tool's own config can pre-select which folder or documents load when the page opens. See [Genie Tools](/workspaces/genie-tools).
- **Genie Tools' spreadsheet blocks now handle up to 500,000 rows** (up from 50,000). The old all-in-one import block is now three focused ones — Import, Files, and Export — alongside four new analysis blocks: KPI, Compare, Anomalies, and Insights. See [Genie Tools](/workspaces/genie-tools).
- **Open a Genie Tool or Genie Agent in a canvas beside the chat**, instead of scrolling past it in the transcript — the one you're building or viewing docks to the right, with a button to switch to another or close it. See [Workspace AI](/workspaces/overview#open-a-genie-tool-or-agent-beside-the-chat).
- **Genie AI can show a knowledge document as a card right in the chat**, with actions to view or download it, instead of just naming it in its reply. See [Knowledge base](/workspaces/overview#knowledge-base).
- **The Genie MCP Server can now browse and import files from a connected cloud drive as knowledge documents**, bringing an external AI client to parity with the web app's Knowledge page. See [Available tools](/workspaces/mcp-api#available-tools).
- **The command palette (⌘K / Ctrl+K) got a full visual redesign** — a single search field flush to the top, and full-width result rows with color-coded icons in place of the old fixed-height panel. See [The command palette](/workspaces/overview#the-command-palette).
- **Pick a whole accent palette for your workspace, not just one color** — it now paints every time-of-day gradient throughout the workspace, not only the sidebar switcher badge. See [Workspace profile](/workspaces/overview#workspace-profile).

## Fixes

- **Fixed multi-step flows (like creating a new workspace) animating in the wrong direction, or visibly changing height, when moving between steps.**

</Update>

<Update label="September 21, 2026" description="Weekly release">

Covers what shipped September 20. Genie Tools becomes a serious data tool — import spreadsheets straight into a workspace table with column matching, export any table as CSV, Excel, or PDF, and draw a chart from one line of description. Genie Agents and Genie Tools also get their own permission categories in custom roles, and knowledge documents now show whether they made it into the search index.

## Updates

- **Import spreadsheets straight into a Genie Tool's workspace table.** Drop one or several files, or a whole folder, and columns are matched to the table automatically — Genie AI steps in only for a heading it can't place on its own. A files list shows everything you've imported, removable one at a time or all at once. See [Genie Tools](/workspaces/genie-tools).
- **Export a Genie Tool's table as CSV, Excel, or PDF.** See [Genie Tools](/workspaces/genie-tools).
- **A new chart block for Genie Tools** turns a table — or any other block's data — into a line, bar, or pie chart with one line of description. See [Genie Tools](/workspaces/genie-tools).
- **A Genie Tool's file upload field now takes any file type**, through a real drag-and-drop zone instead of a fixed image/document allowlist. See [Genie Tools](/workspaces/genie-tools).
- **Genie Agents and Genie Tools get their own permission categories in custom roles** — a workspace can now restrict who may view, build, edit, delete, or run either, separately from Genie Apps. See [Custom roles](/workspaces/members#custom-roles).
- **Knowledge documents show their search-index status**, with a **Retry indexing** action if the semantic index failed to build the first time. See [Knowledge base](/workspaces/overview#knowledge-base).
- **Upload any file type as a project asset**, not just images and video, and drop a whole folder at once. See [Adding images and videos to your app](/projects/overview#adding-images-and-videos-to-your-app).

## Fixes

- **Fixed Genie Agents pausing on insufficient credits with an unreadable error** — the message is now plain, the right wallet is billed, and the agent resumes automatically once you top up credits.
- **Fixed a teammate's message in a shared workspace chat showing in your own message color instead of theirs.**
- **Fixed workspaces you created sometimes appearing under "Others" instead of "Own" in the workspace switcher.**
- **The Genie MCP Server no longer lets a workspace's sole owner accidentally remove their own owner role.**

</Update>

<Update label="September 20, 2026" description="Weekly release">

Covers what shipped September 19. The headline is sharing: a workspace chat can now be shared with just the teammates who need it instead of the whole workspace, and Genie Agents and Genie Tools — private to their creator by default for the first time — follow that same private / selected-people / everyone model, from a settings menu that's now consistent on every card. Genie Agents also get a results page for every run, and Genie AI can rewrite a knowledge document's text in place.

## Updates

- **Share a workspace chat with just the teammates who need it, not the whole workspace.** The chat settings menu now offers **Private**, **Everyone in the workspace**, or **Only selected people** — picked from a member search. In a shared chat, every message now shows who sent it, with their name and avatar next to their turns. See [Workspace chats are private by default](/workspaces/overview#workspace-chats-are-private-by-default).
- **Genie Agents and Genie Tools are private to their creator by default, and share the same way chats do.** Previously any workspace member could open, edit, or delete any agent or tool; now only the owner can, until they share it with the whole workspace or specific people. Every card gets the same settings menu (⋯) for renaming, pausing, sharing, and deleting, wherever that card appears. See [Managing an agent](/workspaces/workflow-apps#managing-an-agent) and [Genie Tools](/workspaces/genie-tools).
- **Genie Agents get a results page for every run.** See the outcome and every step that produced it — status, duration, and what each one output — with an AI step's answer rendered as formatted text instead of raw markdown. A notify step links here by default now. See [Testing an agent](/workspaces/workflow-apps#testing-an-agent).
- **Pin your most-used chats.** Drag a chat onto the top of the sidebar list to pin it, or use the new chat settings menu — pinned chats lead the list under their own heading, ahead of everything else. See [Workspace chats are private by default](/workspaces/overview#workspace-chats-are-private-by-default).
- **Genie AI — and the API and MCP Server — can now rewrite a knowledge document's text in place**, not just its title or description. Ask Genie AI to update a document and it's re-indexed immediately, so search and future builds see the new version right away. See [Knowledge base](/workspaces/overview#knowledge-base).

## Fixes

- **Fixed the project settings dialog's tabs not scrolling when a tab had a lot of content (like a long member list)** — some rows were unreachable, and on a narrow screen the tab content could get pushed out of the dialog entirely.

</Update>

<Update label="September 19, 2026" description="Weekly release">

Covers what shipped September 18. Genie can now be reached from outside the app entirely — link your WhatsApp, Slack, or Microsoft Teams account and message Genie as yourself, or connect one of those accounts for the whole workspace. The Knowledge base also gets automatic folder syncing from a connected Drive, and Genie Tools forms can take a file upload.

## Updates

- **Message Genie from WhatsApp, Slack, or Microsoft Teams.** Link your own account with a one-time code and Genie answers you there with the same tools and history as the web app — or an owner/admin can connect an account for the whole workspace, answered by a fixed member or a Genie Agent. See [Genie on WhatsApp, Slack, and Teams](/workspaces/chat-channels).
- **Keep a Drive folder synced into your knowledge base automatically**, not just imported one file at a time — pick a folder once and Genie mirrors it on demand, and on its own every few hours from then on. See [Knowledge base](/workspaces/overview#knowledge-base).
- **A form in a Genie Tool can now take a file upload**, kept just for that submission or filed straight into your knowledge base. See [Genie Tools](/workspaces/genie-tools).
- **Attach a file to a chat and ask Genie to file it into your knowledge base**, and it files the actual file rather than retyping its contents — the whole document lands intact, not a model's abridged recollection of it. See [Knowledge base](/workspaces/overview#knowledge-base).

</Update>

<Update label="September 18, 2026" description="Weekly release">

Covers what shipped September 16–17. Workspaces gain a third kind of app — [Genie Tools](/workspaces/genie-tools), small internal apps described to Genie AI rather than built — backed by new workspace tables that are also usable as a step inside a Genie Agent. The Knowledge base gets nested folders, whole-folder uploads, and citations on the answers that use it, and you can now connect your own Google Drive.

## Updates

- **Genie Tools: small internal apps, described not built.** Ask Genie AI for a table, a form, a stat, or a button, and it renders live in the chat and on its own page — a table of orders with a form to add one, for example — reading and writing your workspace's own tables, no Genie Cloud project required. See [Genie Tools](/workspaces/genie-tools).
- **Genie Agents get a new Workspace table step**, reading and writing the exact same tables a Genie Tool shows — an agent can gather data overnight and a tool can display it the next morning, nothing to wire up between the two. See [What a Genie Agent is](/workspaces/workflow-apps#what-a-genie-agent-is).
- **Connect your own Google Drive.** Like Gmail, each member connects their own account, and Genie can search files, browse folders, and read Docs, Sheets, and Slides on their behalf. See [Available preset integrations](/workspaces/integrations#available-preset-integrations).
- **Knowledge folders can now hold sub-folders**, and you can upload a whole folder at once — Genie recreates its structure inside Knowledge automatically. See [Knowledge base](/workspaces/overview#knowledge-base).
- **A connected Google Drive is browsable right inside the Knowledge page** — search across the whole drive and import any file Genie can read straight into your knowledge base. See [Cloud drives](/workspaces/overview#cloud-drives).
- **Genie AI's answers now cite the knowledge documents they draw on**, with a clickable citation on each statement and a Sources list under the answer, so you can open (or download) exactly what it read. See [Knowledge base](/workspaces/overview#knowledge-base).

## Fixes

- **Fixed some workspace chats getting permanently stuck after an interactive card (like a confirmation prompt) went unanswered** — later turns could fail outright, or the card itself could stop responding to clicks with no visible error.
- **Fixed the dashboard's quick-action pills (like "New Genie App" or "Ask Genie AI") rendering blank in dark mode** — the button was there, but its label was nearly the same color as its own background until you hovered over it.

</Update>

<Update label="September 17, 2026" description="Weekly release">

Covers what shipped September 15–16. The [Genie MCP Server](/workspaces/mcp-api) catches up to the web app in five places — favoriting a project, Security alerts, and Activity trend stats are all reachable from an external AI client now — and two workspace chat reliability bugs are fixed.

## Updates

- **The Genie MCP Server can now favorite projects, read Security alerts, manage your alert subscription, and read Activity trend stats.** Five new tools — `set_project_favorite`, `get_workspace_security_alerts`, `get_my_security_alert_subscription`, `set_my_security_alert_subscription`, and `get_workspace_activity_stats` — bring an external AI client (Claude Desktop, Cursor, or your own) to parity with the web app in each of those areas. See [Available tools](/workspaces/mcp-api#available-tools).
- **A few existing MCP tools grew fields their web-app counterparts already had.** `update_workspace` can now set an icon and accent color, `update_workspace_mcp_server` can switch a connector between shared and per-member credentials, and `update_guardrails` can turn attachment scanning on or off — all three previously accepted those fields over MCP without applying them. See [Available tools](/workspaces/mcp-api#available-tools).

## Fixes

- **Fixed some workspace chat conversations getting stuck for good with a "cannot be modified" error on every later turn** — most reproducible right after Genie AI created a project mid-conversation. The conversation now recovers and later turns go through normally.
- **Fixed Genie AI's reply cutting out with a stream error when it looked up your connected Gmail or Microsoft 365 tools while building a Genie Agent step.**

</Update>

<Update label="September 15, 2026" description="Weekly release">

Covers what shipped on September 14. A blocked message now reads as a calm notice instead of an error, with an opt-in email alert for admins; Guardrails can check attached images too; the Activity page adds a Security alerts summary and CSV/PDF export; projects get a personal Favorites shelf; and Genie Agents and Integrations both grew new capabilities — Gmail, three new step kinds, and AI-generated cover pictures among them.

## Updates

- **A blocked message now reads "Message held back," not an error.** PII rules and Guardrails already stopped a message that broke a rule; the card now says so plainly, with no "Try again" button. If you can already see that block in the log, you can subscribe to get emailed the moment a new one happens. See [PII rules: Actions](/workspaces/pii-rules#actions) and [Activity log](/workspaces/overview#activity-log).
- **Guardrails now check attachments, not just text.** Turn on **Check images** to judge attached PNG/JPEGs for harmful content, and **Refuse attachments that cannot be checked** to block anything guardrails can't read at all — a PDF, a GIF/WebP image, or any image while image checking is off. See [Guardrails: Scanning attachments](/workspaces/guardrails#scanning-attachments).
- **The Activity page adds a Security alerts panel and CSV/PDF export.** A summary of recent PII and Guardrails blocks sits alongside the insights panel, and the whole log can now be exported as a CSV or PDF from **View all**. See [Activity log](/workspaces/overview#activity-log).
- **Favorite your most-used projects.** Star one from its card (or ask Genie AI) to pin it above the Projects grid, lead the dashboard preview, and filter for it with the new **Favorites** pill — personal to you, and available once a workspace holds three or more projects. See [Recent projects](/workspaces/overview#recent-projects).
- **The workspace switcher lists your own workspaces before ones you've joined**, with your personal default workspace pinned at the very top.
- **Connect Gmail.** Each member connects their own mailbox, and Genie can search mail, read threads, and draft replies on their behalf — the same per-member model Microsoft 365 uses. See [Available preset integrations](/workspaces/integrations#available-preset-integrations).
- **A connector's shared/per-member setting is now editable after you've added it**, from its Edit form, instead of needing to be removed and re-added to change. See [Shared vs. personal connections](/workspaces/integrations#shared-vs-personal-connections).
- **Genie Agents get three new step kinds: Web search, Read a webpage, and Compliance check** — plus Database and Edge function steps can now target any Genie App in your workspace, not just the one an agent is bound to. See [Genie Agents](/workspaces/workflow-apps#the-visual-editor).
- **Genie Agents and Knowledge documents now show an AI-generated cover picture**, drawn from their own name and description, so a list of either is no longer a wall of identical cards. See [Workspaces overview](/workspaces/overview#the-workspace-home).
- **Tracing now names traces with a short, readable reference** (like `genie-demo-142`) instead of a raw id, shown in the URL, the page title, and wherever Genie AI mentions a trace. See [A readable name for every trace](/workspaces/tracing#a-readable-name-for-every-trace).

## Fixes

- **Fixed the Genie AI composer sitting at the transcript's full width even while empty** — it's now narrower and centered until you start typing, and widens back out on the first character.
- **Fixed the chat composer's frosted-glass background not actually blurring the transcript behind it** on some browsers.
- **Fixed a hover anywhere in the sidebar darkening every chat icon in the list**, not just the one under the pointer.

</Update>

<Update label="September 14, 2026" description="Weekly release">

Covers what shipped on September 13. The headline is a redesigned workspace Activity page — search, filters, and a KPI trend chart — alongside Governance view access moving onto custom roles, and a workspace profile you can now brand with a logo and an accent color.

## Updates

- **The workspace Activity page adds search, filters, and a KPI trend chart.** Above the event list, an insights panel shows a KPI row and a trend chart that re-cut instantly by metric and by time range (7, 30, or 90 days); below it, a free-text search and a category filter search the *whole* log on the server, not just the page you're looking at. See [Workspaces overview](/workspaces/overview#activity-log).
- **Governance view access now comes from a role, not a per-member switch.** Grant the **See Governance** permission on a custom role — or on the built-in Member role, workspace-wide — from the Roles page, the same way you'd grant any other permission. The old per-member toggle is gone; regrant access as a role if you'd used it before. See [Who can access Governance](/workspaces/governance#who-can-access-governance).
- **Workspace Settings → General is now an editable workspace profile.** Set a name, description, logo, and accent color for the workspace, all shown everywhere it's named — the sidebar switcher included. See [Workspace profile](/workspaces/overview#workspace-profile).
- **Tracing now records blocked turns.** A message Guardrails or a PII rule refused used to leave no trace at all; it now shows up in Tracing as a **blocked** turn, with which rule blocked it. See [Blocked turns](/workspaces/tracing#blocked-turns).
- **Integrations show what they're doing, in plain language.** Each connected server's row now gives a plain-language status — connected, ready, not working, and so on — with **Connect** right on the row when a sign-in is all that's missing, instead of endpoint URLs and a buried menu. See [Integrations](/workspaces/integrations#manage-connected-servers).
- **Genie AI's multi-page design mock-ups are now swipeable.** When a draft covers more than one page, swipe between them (or use the arrows once you've opened one full-size) — the tabs stay in sync with whichever page is on screen. See [Genie AI can show you a design mock-up](/workspaces/overview#genie-ai-can-show-you-a-design-mock-up).

## Fixes

- **Fixed staging a file in a chat composer shifting the page.** Attachments now show as a compact strip of pills next to the paperclip instead of a tile that grew the input; hover or tap a pill for the full preview.
- **Fixed Governance's German UI showing a verdict that doesn't exist ("Nicht geprüft").**
- **Fixed a live trace's row jumping position on the Tracing timeline while it was still running.**

</Update>

<Update label="September 13, 2026" description="Weekly release">

Covers what shipped since September 12. The headline is a product-wide rename — a **Project** is now called a **Genie App** — alongside members who can now hold more than one role, a Knowledge base redesigned as a familiar file browser, and a more detailed Tracing view.

## Updates

- **A Project is now called a Genie App.** Every screen that used to say "Project" — the sidebar, the dashboard, settings, and Genie AI's own replies — now says "Genie App", matching the name the rest of the product (like the generated app's own transactional email address) already used. Nothing about how they work changes, only the name.
- **Members can hold more than one role at once.** Assign someone **Editor** plus a custom **Billing Manager** role, for example, and they get everything either one allows. See [Members and roles](/workspaces/members#custom-roles).
- **The Knowledge base is redesigned as a familiar file browser.** Folders and documents now behave like Drive: a breadcrumb to navigate, list or grid layouts, sortable columns, and dragging a document straight onto a folder to file it. See [Knowledge base](/workspaces/overview#knowledge-base).
- **Tracing shows more, and shows it better.** A trace's detail view is now resizable, opens as its own fullscreen page, and every id in it — member, project, conversation — links back to that item. Each trace also now shows its **flaggings**: what your PII rules, Guardrails, and Governance rules noticed about that turn. See [Tracing](/workspaces/tracing#flaggings).
- **Genie Agents: the Start card shows — and edits — the real start conditions.** It now names the agent's actual schedule or webhook trigger instead of a generic description, and its Start panel lets you add, retime, or switch off a trigger without describing the change to Genie AI. See [Genie Agents](/workspaces/workflow-apps#editing-triggers-from-the-canvas).
- **Two new previews on the workspace dashboard**, alongside your recent projects: your most recent **Genie Agents**, and a few **Knowledge** documents, each with a quick link to its full page. See [Workspaces overview](/workspaces/overview#recent-projects).
- **Genie AI's confirmation cards — approve, choose, or review something before it proceeds — now appear inline in the chat**, in the order they came up, instead of replacing your prompt input until you answer them.
- **The Genie MCP Server**: `get_workspace_trace` now returns the same flaggings and resolved member/project names the web view shows, and `list_workspace_members` reports every role a member holds, not just the strongest one. See [Genie MCP Server](/workspaces/mcp-api).

## Fixes

- **Fixed the Tracing detail view overflowing its own panel on a narrow (phone-width) screen.**
- **Fixed the workspace chat's message box causing the conversation to jump while you were still typing a longer message.**
- **Fixed a few low-contrast text issues in the command palette and the dashboard.**

</Update>

<Update label="September 12, 2026" description="Weekly release">

Covers what shipped on September 12. Genie AI can now read and update a project's own design guidelines, and open any file in your code directly.

## Updates

- **Genie AI can read and update your project's design guidelines.** Ask "what are this project's brand colors and fonts?" and Genie AI reads `branding.md` — the style guide the build agent checks before every frontend build. Tell it a lasting design decision — "our brand green is #0f9d58, headings are Poppins" — and Genie saves it, creating the guidelines document if the project doesn't have one yet, so it's remembered by every future build instead of just one build brief. See [Genie AI can read and update your project's design guidelines](/workspaces/overview#genie-ai-can-read-and-update-your-projects-design-guidelines).
- **Genie AI can open any file in your project directly**, and now reads the files most relevant to a code question in full rather than a short snippet — so "how does checkout work?" gets an answer grounded in the whole file, not just the matched lines. See [Genie AI answers with the actual code](/workspaces/overview#genie-ai-answers-with-the-actual-code).
- **Both are now available through the Genie MCP Server** too, so an external AI client can read a project's files and its design guidelines the same way. See [Genie MCP Server](/workspaces/mcp-api).

</Update>

<Update label="September 11, 2026" description="Weekly release">

Covers what shipped on September 11. Knowledge documents can now be sorted into folders.

## Updates

- **Folders for knowledge documents.** File a document into a folder to make it easier to find, from the chip rail above the document list in the Knowledge page or a project's Settings → Knowledge tab. Folders are separate from a document's scope — they're just for finding things, not for deciding what Genie AI can use — so the same folder can hold both workspace-wide and project-specific documents. Each folder holds up to 200 documents, and deleting a folder never deletes what's inside it. See [Knowledge base](/workspaces/overview#knowledge-base).
- **The Genie MCP Server now covers knowledge-document folders**, so an external AI client can list, create, rename, delete, and file into them. See [Genie MCP Server](/workspaces/mcp-api).

</Update>

<Update label="September 10, 2026" description="Weekly release">
...
- **Guardrails: control what Genie AI will discuss.** Teams-plan workspaces can now turn on guardrails from workspace settings — dial the strength of harmful-content filters, name off-limits topics, and block specific words — all configured in plain language. See [Guardrails](/workspaces/guardrails).
- **Governance: review projects and enforce compliance rules.** A new workspace settings page lets owners and admins approve, flag, or disable individual projects, see every integration and knowledge document shared workspace-wide, and write natural-language compliance rules — with built-in templates for things like GDPR and the EU AI Act — that re-check every project automatically whenever it ships. See [Governance](/workspaces/governance).
- **Tracing: see every Genie AI turn in your workspace.** A new admin page lists every conversation turn any member has had with Genie AI — model, tokens, credits, timing, and every tool call — with a detailed view of any individual turn, gated behind its own permission. See [Tracing](/workspaces/tracing).
- **Genie Agents can now be tested for real.** A **Test run** button starts an actual run of your saved agent and paints live status on every step as it goes, and **Test this step** runs a single connected-app step on its own with realistic example values — no need to save or run the whole flow first. If a step's test fails, hand it to Genie AI with one click and it proposes a fix. See [Testing an agent](/workspaces/workflow-apps#testing-an-agent).
- **Genie itself is now a built-in connected app for Genie Agents** — nothing to set up, always available — so a step can create a project, send a follow-up prompt, or change a workspace setting as part of an automation. See [Genie Agents](/workspaces/workflow-apps).
...
- **The Genie MCP Server now covers Guardrails, Governance, and Tracing**, so an external AI client has the same access an admin has from the settings pages. See [Genie MCP Server](/workspaces/mcp-api).
...
<Update label="September 9, 2026" description="Weekly release">
...
- **Workspace and personal settings are now one surface.** They used to be two unrelated screens — workspace settings inside the workspace, personal settings as a bare page with no way back. Open either from the account menu at the bottom of the sidebar and you get the same navigation, with the other half always one click away. See [Account settings](/account/settings).
- **Custom roles.** Beyond the four built-in roles, owners and admins can now build custom roles that narrow exactly what a role can do — remove billing access, keep member management, drop the ability to delete projects — from a catalog of granular permissions. See [Custom roles](/workspaces/members#custom-roles).
...
- **Workflow apps are now called Genie Agents.** You can now build one directly from the **Genie Agents** page — give it a name and a first step and you land straight in the editor, without describing it to Genie AI first. Rename, pause/resume, or delete an agent from its row menu, and switch between a card or table view of your agents. See [Genie Agents](/workspaces/workflow-apps).
- **Microsoft 365 (Outlook, OneDrive, SharePoint) can now be connected** as a workspace integration — a personal, per-member connection like the other mailbox-style connectors. See [Integrations](/workspaces/integrations).
...
<Update label="September 8, 2026" description="Weekly release">
...
- **Workflow apps: a new kind of app with no screen of its own.** Build a flow of steps — query your Genie Cloud database, call an external API, ask the AI to do something, run one of your project's own backend functions, or call another workflow app — configured by describing what you want to Genie AI, with a visual editor if you'd rather see and adjust the flow by hand. Run it on a schedule or trigger it from an incoming webhook. See [Workflow apps](/workspaces/workflow-apps).
- **Talk to Genie AI from your own code.** Workspaces can now mint API keys so a backend, a cron job, or a bot can send messages to your workspace's Genie AI over HTTP — billed to the workspace, and a real conversation your team can see and reply to, not a separate assistant. See [Workspace API keys](/workspaces/api-keys).
- **A new project can now share an existing one's Genie Cloud backend** instead of starting with an empty database — useful when two apps are really facets of the same product and should see the same customers and data. Each app still deploys and bills separately; only the data is shared. See [Genie Cloud](/cloud/overview).
...
- **Per-member connections for personal integrations**, like a mailbox: an admin still sets the connector up once for the workspace, but each teammate connects their own account, and Genie always acts using the person talking to it — never falling back to someone else's connection. See [Integrations](/workspaces/integrations).
...
- **Claude (Anthropic) models are now available through Genie's built-in AI relay**, alongside OpenAI, for your project's own backend code — no separate Anthropic account needed. See [Built-in AI](/cloud/ai-and-email).
- **Your app can now send spreadsheet attachments through Genie's built-in AI.** An `.xlsx` file passed to one of your project's own AI features is automatically converted to a plain-text table before reaching the model — no AI model reads a spreadsheet directly, so this happens with no setup on your end. See [Built-in AI](/cloud/ai-and-email#spreadsheet-attachments).
...
- **Genie MCP Server: a follow-up prompt sent to a project that's already building now queues automatically** instead of failing outright, matching how the web app already behaves. See [Genie MCP Server](/workspaces/mcp-api).
...
<Update label="September 5, 2026" description="Weekly release">
...
- **Genie MCP Server: `generate_design_mock` now takes a layout and color direction, matching the in-app design mock-up tool** — external MCP clients previously had no way to steer either, so a mock-up's direction was left entirely to the image model. `brief_design_mock_for_build` also now returns a ready-to-paste design section. See [Genie MCP Server](/workspaces/mcp-api).
...
<Update label="September 4, 2026" description="Weekly release">
...
<Update label="September 2, 2026" description="Weekly release">
...
- **Fixed an approved design mock-up's picture not actually carrying into the build.** Genie AI only handed the build a written change list, so what got built could drift from the mock-up you saw and approved. The image itself now joins the build the same way an uploaded attachment does. See [Genie AI can show you a design mock-up](/workspaces/overview#genie-ai-can-show-you-a-design-mock-up).
- **Every choice card can now take a typed answer, not just a pick from its options.** The single-choice list card had no way out if none of its options matched what you meant, other than canceling; it now has the same free-text field the checklist card already offered. See [Interactive answers](/workspaces/overview#interactive-answers).
...
<Update label="September 1, 2026" description="Weekly release">
...
- **Genie AI can now search the web.** When a question isn't answered by Genie's own knowledge or your workspace — a third-party API's current behavior, a competitor, a price Genie can't otherwise know — it now runs a live web search or reads a specific page for you, instead of guessing. See [Genie AI can search the web](/workspaces/overview#genie-ai-can-search-the-web).
...
<Update label="August 27, 2026" description="Weekly release">
...
- **Fixed switching a project's workspace from Project Settings just navigating you to the other workspace instead of actually moving the project there.** See [Workspace](/projects/settings#workspace).
...
<Update label="August 25, 2026" description="Weekly release">
...
- **Genie AI now drafts a design concept before creating a brand-new project.** Ask it to build something new and Genie AI sketches the key screens first — no existing app needed — lets you react, and carries the direction you approve straight into the very first build. See [Genie AI can show you a design mock-up](/workspaces/overview#genie-ai-can-show-you-a-design-mock-up).
...
<Update label="August 24, 2026" description="Weekly release">
...
- **Fixed workspace owners occasionally being unable to view or manage their own project's knowledge base documents.** See [Knowledge base](/workspaces/overview#knowledge-base).

</Update>

<Update label="August 22, 2026" description="Weekly release">
...
- **Genie MCP Server: drafting a design mock-up before a build is now the default here too**, for external MCP clients — matching the in-app chat behavior from August 21. See [Genie MCP Server](/workspaces/mcp-api).
...
<Update label="August 21, 2026" description="Weekly release">
...
<Update label="August 20, 2026" description="Weekly release">
...
<Update label="August 19, 2026" description="Weekly release">

This update catches up two knowledge base fixes from August 19 that were missed in the original pass.

## Fixes

- **Fixed a workspace admin's "Applies to" dropdown in Knowledge base sometimes opening blank instead of listing every project they administer** — including the project the admin was standing in when they opened it. Filing a document into a project is now also checked against the workspace it's filed under, so a project from the wrong workspace can no longer be picked. See [Knowledge base](/workspaces/overview#knowledge-base).
...
<Update label="August 18, 2026" description="Weekly release">
...
- **Genie's suggestion for what to build next now appears right above your message box** the moment a build finishes, instead of only at the bottom of a build card you've likely already scrolled past. Tap it to see the ideas as a checklist, tick the ones you want, and **Apply** puts a ready-to-send prompt in your message box without sending it. See [Genie suggests what to build next](/workspaces/overview#genie-suggests-what-to-build-next).
- **Genie AI now checks with you before guessing at an ambiguous request** — like which project you mean, or whether "add" means something new or a replacement — instead of building the wrong thing. It also now recognizes when you're asking for the same thing again, however differently worded, and stops to check what happened last time rather than repeating the same attempt. See [Genie checks before guessing](/workspaces/overview#genie-checks-before-guessing).
- **You can now attach Excel spreadsheets (.xlsx) to a Genie AI chat message**, alongside the existing plain text, Markdown, CSV, JSON, and Word documents — Genie reads the sheet contents as a table. See [Attachments carry into your build](/workspaces/overview#attachments-carry-into-your-build).

## Updates

- **Genie MCP Server: project memory is now available to external clients.** `save_project_memory`, `search_project_memory`, and `forget_project_memory` let an external AI client read and write the same shared project notes Genie AI keeps in the workspace chat. See [Genie MCP Server](/workspaces/mcp-api).
...
<Update label="August 17, 2026" description="Weekly release">
...
<Update label="August 16, 2026" description="Weekly release">
...
<Update label="August 15, 2026" description="Weekly release">
...
- **Genie MCP Server: `create_project` and `send_followup_prompt` now return a run id.** Pass it to `get_project_build_status` to poll the exact build turn you just started, instead of always getting the project's newest one — handy if you fire off more than one prompt in a row. See [Genie MCP Server](/workspaces/mcp-api).
...
<Update label="August 14, 2026" description="Weekly release">
...
<Update label="August 13, 2026" description="Weekly release">
...
<Update label="August 12, 2026" description="Weekly release">
...
- **Genie AI answers with the actual code.** Ask "how does checkout work in my app?" and Genie AI now reads the real code behind its answer instead of guessing from file and function names — so it describes what your code actually does, not just what its name suggests. See [Genie AI can see and debug your project](/workspaces/overview#genie-ai-answers-with-the-actual-code).
...
<Update label="August 11, 2026" description="Weekly release">
...
<Update label="August 10, 2026" description="Weekly release">
...
<Update label="August 9, 2026" description="Weekly release">
...
- **Genie AI can show you a design mock-up before it builds anything.** Ask something like "what would this look like with a warmer palette?" and Genie AI screenshots your real app, has an image model repaint it with the change you described, and shows you both pictures — with a clear note that a mock-up is an impression, not a pixel-accurate render. React to what you see and it iterates, or turns an approved direction into a build once you're ready. See [Genie AI can show you a design mock-up](/workspaces/overview#genie-ai-can-show-you-a-design-mock-up).
- **Genie MCP Server: design mock-ups.** External AI clients can now generate, list, re-fetch, and iterate on a project's design mock-ups with five new tools, mirroring the workspace AI feature above — images are returned as native MCP image content. See [Genie MCP Server](/workspaces/mcp-api).

## Updates

- **Genie's built-in image generation can now produce logos and wordmarks with real, legible text.** Previously every AI-generated image suppressed text entirely, so a logo request came back as vague shapes. Ask for one by name and Genie renders the actual lettering. See [Built-in AI, images, and email](/cloud/ai-and-email).
...
<Update label="August 8, 2026" description="Weekly release">
...
- **A project's Genie AI conversation is now one shared thread for your whole team.** Previously, each teammate who opened a project's Genie AI panel got their own separate conversation — now everyone lands in the same thread, so nobody has to catch up before they can help. See [The Genie AI panel](/projects/overview#the-genie-ai-panel).
- **Workspace chats are shared with your team too.** A link to a teammate's Genie AI conversation now opens for you instead of failing with "not found," and workspace admins can open, rename, or delete any conversation in the workspace for support. Browsing your own **Recent chats** is unchanged. See [Workspace AI](/workspaces/overview#workspace-chats-are-shared-with-your-team).
- **"Who else is here" — a badge for who's currently viewing a project.** The project's Genie AI tab now shows who else in your workspace has the project open right now, alongside the existing build-status badge. See [The Genie AI panel](/projects/overview#who-else-is-here).
- **Genie MCP Server: see who's viewing a project, and read/rename/delete any workspace chat.** The new `list_project_viewers` tool mirrors the viewer badge. `get_workspace_chat`, `rename_workspace_chat`, and `delete_workspace_chat` now work on any chat in the connected workspace, not just ones the caller started, matching the shared-chat change above. See [Genie MCP Server](/workspaces/mcp-api).
...
<Update label="August 7, 2026" description="Weekly release">
...
- **Mobile project pages are now built around the Genie AI tab.** On mobile, Genie AI is the primary way to work on a project; the project's own build chat is now a secondary, admin-only view. See [The Genie AI panel](/projects/overview#the-genie-ai-panel).
...
<Update label="August 6, 2026" description="Weekly release">
...
<Update label="August 4, 2026" description="Weekly release">
