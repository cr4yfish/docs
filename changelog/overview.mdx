...
- **MCP Server: member emails and workspace owner.** `list_workspace_members` now returns each member's email address and display name alongside their role, and `get_workspace_info` now includes the workspace owner. See [Genie MCP Server](/workspaces/mcp-api).
