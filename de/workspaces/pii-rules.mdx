...
<Tabs>
  ...
  <Tab title="Blockieren">
    Die Nachricht wird abgelehnt, bevor sie die KI erreicht. Die betroffene Person sieht einen schlichten Hinweis „Nachricht zurückgehalten" — keine Fehlermeldung —, der nennt, welche Datenkategorie die Blockierung ausgelöst hat (zum Beispiel „Deine Nachricht enthielt eine Kreditkartennummer"), ohne Wiederholen-Button: Denselben Inhalt erneut zu senden, würde ihn nur erneut blockieren.

    Blockierte Nachrichten werden ebenfalls (geschwärzt) protokolliert, damit Admins sie prüfen können, und wer dieses Protokoll ohnehin schon einsehen kann, kann **E-Mail-Benachrichtigungen** für neue Blockierungen einschalten — siehe [Aktivitätsprotokoll](/de/workspaces/overview#aktivitätsprotokoll).
