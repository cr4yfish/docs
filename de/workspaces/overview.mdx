...
<Note>
Seit dem [Update vom 13. September 2026](/de/changelog/overview) nennt die Produktoberfläche ein Projekt **Genie App** statt „Projekt". Diese Seiten sagen weiterhin „Projekt" — gemeint ist so oder so dasselbe.
</Note>
...
<CardGroup cols={2}>
  <Card title="Workspace-Umschalter" icon="arrow-left-right">
    Workspace-Name und Tarif erscheinen oben in der Seitenleiste. Klicke darauf, um zu einem anderen Workspace zu wechseln oder einen neuen zu erstellen — eigene Workspaces stehen vor denen, in die du eingeladen wurdest, mit deinem persönlichen Standard-Workspace ganz oben.
  </Card>
  ...
  <Card title="Genie Agents" icon="workflow">
    Jeder [Genie Agent](/de/workspaces/workflow-apps) im Workspace — Backend-Automatisierungen ganz ohne eigene Oberfläche.
  </Card>
  <Card title="Letzte Chats" icon="message-square">
...
Workspace-Einstellungen und deine persönlichen Einstellungen teilen sich eine Navigation, erreichbar über die Kontozeile unten in der Seitenleiste: Wähle **Workspace-Einstellungen** (Allgemein, Mitglieder, Rollen, Integrationen, PII-Erkennung, Guardrails, Governance sowie — für Admins und Eigentümer — Aktivitäten, Tracing und Abrechnung) oder **Persönliche Einstellungen** (Erscheinungsbild, Anmeldemethoden, Benachrichtigungen). Beide ersetzen die Seitenleiste durch eine Einstellungs-Navigation mit einem Zurück-Pfeil zum Workspace. Siehe [Workspace-Mitglieder einladen und verwalten](/de/workspaces/members) für Rollen und [Kontoeinstellungen](/de/account/settings) für deine persönlichen Präferenzen.

### Workspace-Profil

Auf der Einstellungsseite **Allgemein** bearbeitet ein Eigentümer oder Admin den Workspace selbst: Name, Beschreibung, ein Logo und eine Akzentpalette — gemeinsam gespeichert und überall sichtbar, wo der Workspace benannt wird, vom Umschalter in der Seitenleiste bis zum Drawer auf dem Mobilgerät. Die Palette ist ein ganzes Farbset, kein einzelner Farbton: Sie bemalt jeden tageszeitabhängigen Verlauf im gesamten Workspace, nicht nur das Badge im Umschalter. Lässt du Logo oder Palette leer, fällt das Badge wie bisher auf den Anfangsbuchstaben des Workspace auf einem Standardhintergrund zurück, und die Verläufe folgen der Standard-Tageszeitrotation.

<Tip>
Lass das stattdessen von Genie AI erledigen — *„setze unser Workspace-Logo auf dieses Bild"* oder *„wechsle unsere Akzentpalette zu etwas Wärmerem"* — über den **Genie AI fragen**-Button auf der Seite „Allgemein".
</Tip>
...
Darunter folgen zwei weitere reine Vorschauen im selben Stil: deine zuletzt verwendeten **Genie Agents** und einige Dokumente aus der **Knowledge**-Datenbank deines Workspace — jeweils mit einer eigenen „Genie AI fragen"-Abkürzung und einem Link zur vollständigen Seite. Keine der beiden Vorschauen kann selbst einen Agenten ausführen oder ein Dokument verwalten; sie dienen nur dem schnellen Blick auf dem Weg zur Seite, zu der sie gehören.

Sowohl die Karte eines Agenten als auch die Zeile eines Dokuments tragen ein Titelbild, automatisch generiert aus Name und Beschreibung, sodass eine Liste von Agenten oder Dokumenten nicht mehr aus lauter identischen Karten besteht. Es entsteht in dem Moment, in dem du einen Agenten oder ein Dokument anlegst, oder beim ersten Öffnen eines älteren, das es von vor dieser Funktion gibt — du musst es nicht selbst anstoßen.

Markiere ein Projekt über einen Stern auf seiner Karte (oder bitte Genie AI darum) als Favorit, um es griffbereit zu halten: favorisierte Projekte bekommen ein eigenes Regal oberhalb des Projekte-Rasters, führen diese Dashboard-Vorschau an und lassen sich auf der Projekte-Seite über den Filter **Favoriten** auswählen. Ein Favorit ist persönlich — Teamkolleg:innen behalten eigene Regale — und die Funktion schaltet sich erst ab drei oder mehr Projekten im Workspace ein, weil das Raster darunter ohnehin schon alles auf einen Blick zeigt.
...
Die vollständige Aktivitätsprotokoll-Seite ergänzt ein Insights-Panel oberhalb der Ereignisliste: eine Kennzahlen-Reihe und ein Trenddiagramm, beide sofort neu berechnet über ein Metrik-Dropdown (Ereignisse, aktive Mitglieder oder eine bestimmte Kategorie) und einen Zeitraum (7, 30 oder 90 Tage). Darunter durchsuchen eine Freitextsuche und ein Kategorie-Filter das gesamte Protokoll serverseitig — nicht nur die gerade angezeigten Zeilen —, sodass eine Suche nach dem Namen einer Kollegin oder eines Kollegen alles findet, was sie oder er getan hat, egal wie weit es zurückliegt.

Ein **Sicherheitswarnungen**-Panel steht daneben und fasst kürzliche Blockierungen aus deinen [PII-Regeln](/de/workspaces/pii-rules) und [Guardrails](/de/workspaces/guardrails) zusammen — sichtbar für alle, die das zugrunde liegende Protokoll ohnehin schon einsehen können. Von demselben Panel aus kann jede Person mit diesem Zugriff sich per E-Mail benachrichtigen lassen, sobald eine neue Blockierung passiert — standardmäßig ausgeschaltet. Das gesamte Aktivitätsprotokoll lässt sich außerdem über **Alle anzeigen** als CSV oder PDF exportieren.
...
Frag „wie funktioniert der Checkout in meiner App?", und Genie AI liest jetzt den echten Code hinter seiner Antwort, statt anhand von Datei- und Funktionsnamen zu raten — es beschreibt also, was dein Code tatsächlich tut, und nicht nur, was sein Name vermuten lässt. Für die Dateien, die für deine Frage am wichtigsten sind, liest sie jetzt die ganze Datei statt nur eines kurzen Ausschnitts, und du kannst sie auch bitten, eine bestimmte Datei direkt zu öffnen — „zeig mir App.tsx" oder „was steht in der Login-Seite?"
...
Umfasst ein Entwurf mehrere Seiten, wische zwischen ihnen hin und her — oder nutze die Pfeile, sobald du eine in voller Größe geöffnet hast —, genau wie beim Durchblättern von Fotos. Die Tabs über dem Bild bleiben synchron mit der jeweils angezeigten Seite, und in voller Größe geöffnet kannst du durch den ganzen Satz blättern, Vorher-Nachher inklusive, ohne den Viewer zu schließen und neu zu öffnen.
...
### Genie AI kann die Design-Richtlinien deines Projekts lesen und aktualisieren

Frag *„was sind die Markenfarben und Schriftarten dieses Projekts?"*, und Genie AI liest `branding.md` — den Styleguide, den der Build-Agent prüft, bevor er Frontend-Code schreibt: Farben, Typografie, Vorgaben für Komponenten und was erlaubt beziehungsweise tabu ist. Sag ihr eine dauerhafte Design-Entscheidung — *„unser Markengrün ist #0f9d58, Überschriften sind Poppins, nie reines Schwarz verwenden"* —, und Genie AI speichert sie und legt das Richtliniendokument an, falls das Projekt noch keins hat — so bleibt im Chat Festgelegtes für jeden künftigen Build erhalten, statt nur in einem einzigen Build-Briefing zu landen.

<Note>
Neue Richtlinien zu speichern gestaltet deine Live-App nicht von selbst um — es ändert nur, wonach sich dein *nächster* Build richtet. Bitte Genie um einen neuen Build, wenn du den neuen Look sofort angewendet haben willst.
</Note>

### Anhänge fließen in deinen Build ein

Hängst du beim Bitten der Workspace-KI, ein Projekt zu erstellen oder zu aktualisieren, ein Bild oder PDF an deine Nachricht an — einen Entwurf, ein Referenzfoto —, geht die Datei selbst in diesen Build ein, nicht nur eine Textbeschreibung davon. Über Bilder und PDFs hinaus kannst du auch reinen Text, Markdown, CSV, JSON, Word-Dokumente und Tabellen anhängen — Excel-Arbeitsmappen und Apple-Numbers-Dateien kommen beide mit ihren Zellbezügen und Formeln an, nicht nur mit den berechneten Werten —, und ein iPhone-HEIC-Foto wird automatisch umgewandelt, damit seine Vorschau korrekt angezeigt wird.
...
### Ein Genie Tool oder ein Agent neben dem Chat öffnen

Auf der vollständigen Chat-Seite scrollt ein [Genie Tool](/de/workspaces/genie-tools), das gebaut oder geöffnet wird, oder ein [Genie Agent](/de/workspaces/workflow-apps) nicht mehr einfach im Gesprächsverlauf vorbei — seine Karte dockt rechts an eine Canvas an, während die Unterhaltung links bleibt. Ist mehr als eines geöffnet, wechselt ein Button im Kopf jeder Karte die Canvas auf diese Karte; das Schließen der Canvas (oder dieser Button auf der angedockten Karte) legt sie wieder in die Unterhaltung zurück. Auf dem Handy oder in einem schmaleren Fenster bleiben Karten wie bisher im Gesprächsverlauf.

Ein Genie Agent ohne Verzweigung wird auf der Canvas von oben nach unten gezeichnet — Start, jeder Schritt, Ergebnis, eine Zeile pro Karte — statt im seitlichen Raster des Editors, damit er in der schmalen, hohen Spalte der Canvas sauber lesbar bleibt. Ein Agent mit einer Verzweigung, oder einer, den du im Editor von Hand angeordnet hast, behält das Raster.
...
Die Oberfläche ist wie ein vertrauter Dateimanager aufgebaut: Ordner und unabgelegte Dokumente liegen an der Wurzel, das Öffnen eines Ordners zeigt seinen Inhalt, und ein Breadcrumb führt zurück nach oben — nichts wird je an zwei Stellen zugleich aufgelistet. Die Suche verlässt die Ordneransicht komplett und durchsucht alles auf einmal, sodass das Finden eines Dokuments nie davon abhängt, sich zu merken, wo es abgelegt wurde. Wechsle zwischen Listen- und Kachelansicht (für das nächste Mal gemerkt), sortiere nach jeder Spalte, und zieh ein Dokument direkt auf einen Ordner, um es dort abzulegen — oder auf den Breadcrumb **Knowledge**, um es wieder herauszunehmen — statt den Verschieben-Dialog zu öffnen.

Lade reinen Text, Markdown, CSV, JSON, YAML, XML, HTML, Quellcode-Dateien, Word (`.docx`), Excel (`.xlsx`) oder PDF hoch — bis zu 10 MB pro Datei. Ein gescanntes PDF ohne Textebene wird jetzt automatisch per OCR gelesen, statt abgelehnt zu werden. Bilder werden im Uploader weiterhin nicht direkt angenommen, da ihr Inhalt nicht im Browser extrahiert werden kann; schick eins stattdessen im Chat an Genie AI und bitte sie, es für dich abzulegen. Du kannst auch einen ganzen Ordner auf einmal hochladen — Genie legt seine Struktur als Knowledge-Ordner nach, überspringt dabei, was sie nicht lesen kann, statt den gesamten Upload abzubrechen.

Jedes Dokument zeigt seinen eigenen Suchindex-Status — indexiert, nur nach Stichwort durchsuchbar, falls der semantische Index nicht aufgebaut werden konnte, oder noch nicht indexiert — mit einer Aktion **Indexierung wiederholen** direkt daneben, wenn sie gebraucht wird.

Bevor Genie AI ein Projekt baut oder aktualisiert, prüft sie automatisch die für dieses Projekt geltende Wissensdatenbank — die workspaceweiten Dokumente plus alles, was nur für dieses Projekt abgelegt wurde — und lässt das Gefundene in den Build einfließen. Dabei markiert die Antwort mit einer kleinen Quellenangabe genau, aus welcher Textstelle jede Aussage stammt — klick darauf, um das Quelldokument an dieser Stelle zu öffnen, oder öffne die Liste **Quellen** unter der Antwort, um jedes verwendete Dokument zu sehen (und herunterzuladen). Genie AI kann ein Dokument auch allein über seinen Dateinamen oder seine Beschreibung finden, nicht nur über das, was darin geschrieben steht — nützlich für ein Dokument, dessen Inhalt vor allem aus Zahlen oder einem Scan besteht, etwa eine Tabelle mit Kennzahlen oder ein unterschriebener Vertrag. Bitte sie, ein Dokument direkt anzuzeigen oder zu öffnen, und es erscheint als Karte direkt im Chat, mit Aktionen zum Ansehen oder Herunterladen, statt nur in der Antwort genannt zu werden.

Jedes Dokument trägt außerdem ein **Datum, um das es geht**, das überall dort steht, wo es aufgelistet oder zitiert wird — getrennt vom Tag des Uploads. Genie ermittelt es automatisch: ein vollständiges Datum im Dateinamen oder Titel, sonst der Zeitpunkt der letzten Änderung im verbundenen Laufwerk, und andernfalls der Upload-Tag als Ersatz. Korrigiere es selbst im **Umbenennen oder verschieben**-Dialog des Dokuments, oder bitte Genie AI, das Dokument zu lesen und das Datum für dich zu setzen — deine Korrektur bleibt über jeden späteren Abgleich oder jede Bearbeitung hinweg erhalten, bis du sie wieder auf automatisch zurückstellst. Dasselbe steht über den [Genie-MCP-Server](/de/workspaces/mcp-api#verfügbare-tools) und die Workspace-API zur Verfügung.

Innerhalb eines Geltungsbereichs lassen sich Dokumente außerdem in **Ordner** einsortieren, die jetzt auch eigene Unterordner enthalten können — eine Ablage-Ebene zum Wiederfinden, getrennt vom Geltungsbereich, der entscheidet, was Genie AI nutzen darf. Ein Ordner „Marke" kann sowohl ein workspaceweites Dokument als auch das eigene Textkonzept eines Projekts enthalten, und ob ein Dokument in einem Ordner liegt oder nicht, ändert nie, was Genie bei einer Suche findet. Lege Ordner über die Chip-Leiste oberhalb der Dokumentliste an, benenne sie um oder lösche sie, oder zieh einen Ordner auf einen anderen, um ihn zu verschachteln; jeder fasst direkt bis zu 200 Dokumente, und das Löschen eines Ordners löscht nie, was darin oder darunter liegt — diese Dokumente werden einfach unabgelegt.

Bitte Genie AI, ein Dokument zu aktualisieren, und sie kann seinen ganzen Text direkt überschreiben, nicht nur Titel oder Beschreibung ändern — der neue Text wird sofort neu indexiert, sodass Suche und der nächste Build ihn gleich sehen. Dasselbe ist über den [Genie-MCP-Server](/de/workspaces/mcp-api#verfügbare-tools) und die Workspace-API verfügbar. Ein Dokument, das aus einem verbundenen Cloud-Speicher gespiegelt wurde, lässt sich so nicht überschreiben, da der nächste Abgleich die Änderung überschreiben würde — ändere es stattdessen an der Quelle.

### Cloud-Speicher

Ein Panel **Cloud-Speicher** am Ende der Knowledge-Seite zeigt für jedes Mitglied, wo es mit Google Drive oder Microsoft 365 (OneDrive und SharePoint) steht — noch nicht verbunden, oder verbunden und bereit zum Durchsuchen. Sobald du dein eigenes Konto verbunden hast (siehe [Verfügbare Preset-Integrationen](/de/workspaces/integrations#verfügbare-preset-integrationen)), erscheint es als eigener Ort im Knowledge-Browser: durchsuche das gesamte Laufwerk — bei Microsoft 365 dein OneDrive oder jede SharePoint-Site, der du folgst —, öffne Ordner, und klicke bei jeder Datei, die Genie lesen kann, auf **In Knowledge importieren**, um eine Kopie als Wissensdokument abzulegen — zitierfähig wie jedes andere.

Über das einzelne Importieren hinaus kannst du auch einen ganzen Ordner **im Abgleich** halten — einen Drive-Ordner, oder eine OneDrive-/SharePoint-Bibliothek bzw. einen Ordner darin: Wähle ihn einmal aus der Liste **Synchronisierte Ordner** (oder nur die Unterordner, die du willst), und Genie spiegelt ihn ab dann automatisch in deine Wissensdatenbank — neue und geänderte Dateien werden übernommen, per **Jetzt synchronisieren** jederzeit auf Wunsch und von allein alle paar Stunden dazwischen. Verschiebst du eine Datei zwischen zwei synchronisierten Ordnern, bleibt sie dasselbe Wissensdokument, statt neu abgelegt zu werden; entfernst du eine Datei aus der Quelle, wird ihr Dokument dafür nicht gelöscht — es wird als **Aus dem Laufwerk entfernt** markiert, fällt aus der Suche und aus Genie AIs Antworten heraus und kehrt zurück, sobald die Datei es tut. Sieben Tage lang bleibt es für diesen Fall erhalten, danach wird es endgültig gelöscht.

<Tip>
Bitte Genie AI zum Beispiel *„lege unsere Markenrichtlinien als Wissensdokument ab"* oder *„was haben wir zur Zahlungs-API hinterlegt?"* — sie kann Wissensdokumente in deinem Namen suchen, lesen, hinzufügen, umbenennen, verschieben, löschen und in Ordner einsortieren, genau wie du es auf der Knowledge-Seite selbst tun kannst.
</Tip>

### Wissensmodell

Über Suche und Zitate hinaus kann Genie AI auch ein strukturiertes Modell dessen aufbauen, worum es in deiner Wissensdatenbank eigentlich geht — die **Entitäten** darin (eine Person, eine Organisation, ein Vertrag, eine Entscheidung) und die **Beziehungen** zwischen ihnen, jede davon bis zur genauen Textstelle zurückverfolgbar, aus der sie stammt.

Aktiviere **Dokumente automatisch lesen** am Ende der Knowledge-Seite (nur Owner und Admins, standardmäßig aus), und Genie liest jedes Dokument im Workspace und schlägt die gefundenen Entitäten, Werte und Verknüpfungen vor — neue und geänderte Dokumente werden ab dann automatisch erfasst. Jeder im Workspace kann das Ergebnis im Panel **Klassen und Beziehungen** weiter unten auf derselben Seite durchsuchen: die Arten von Dingen, die erfasst werden (Personen, Organisationen, Entscheidungen, Begriffe und jede von einem Admin hinzugefügte eigene Klasse), wie viele Eigenschaften jede hat und wie viele Entitäten du sehen kannst. Öffne eine Klasse, um ihre Entitäten zu sehen, jeweils mit dem Hinweis **Vorgeschlagen**, bis sie bestätigt sind, und **Teilweise verborgen**, wo ein Dokument, das du nicht lesen kannst, mehr dazu sagt.

<Tip>
Bitte Genie AI zum Beispiel *„was wissen wir über die Acme GmbH?"* oder *„führe diese beiden doppelten Kontakte zusammen"* — das Prüfen der Extraktionsergebnisse, das Zusammenführen oder Aufteilen von Entitäten und Fragen zum Modell laufen über den Chat mit Genie AI, nicht über eine eigene Übersichtsseite.
</Tip>

Admins können Klassen und Beziehungstypen auch selbst anlegen und bearbeiten — Name, Beschreibung, Eigenschaften und welche Klassen eine Beziehung verbindet — direkt im selben Panel, und können alle Dokumente von Grund auf neu lesen lassen oder das Modell komplett zurücksetzen, falls ein sauberer Neustart nötig ist.

### Zugriffsbereiche

Standardmäßig kann jedes Workspace-Mitglied jeden Knowledge-Ordner sehen. Enthält ein Ordner etwas Sensibleres — Personalunterlagen, die Dateien eines einzelnen Kunden —, kann ein Owner oder Admin ihn über die Workspace-Einstellungen → **Knowledge-Zugriff** in einen **Zugriffsbereich** einschließen: einen Bereich anlegen, festlegen, ob er standardmäßig für alle Mitglieder offen ist oder nur für von dir freigegebene Personen, und dann die Rollen, Gruppen oder einzelnen Mitglieder hinzufügen, die Zugriff haben sollen, und den Ordner in den Bereich legen. Wer außerhalb der Reichweite eines Bereichs steht, verliert nicht nur den Zugriff auf den Ordner — er sieht nicht einmal, dass er existiert.

Ein Bereich kann **Lese**-Zugriff gewähren (das Enthaltene sehen und nutzen) oder **Schreib**-Zugriff (zusätzlich Dokumente darin hinzufügen, verschieben und löschen). Einen Ordner in einen Bereich zu legen oder herauszunehmen, oder festzulegen, wer ihn erreicht, gilt für alles darin, einschließlich Unterordner.

<Tip>
Gib lieber einer Rolle oder Gruppe Zugriff, statt einzelne Mitglieder nacheinander zu benennen — das lässt sich später leichter überprüfen und anpassen, und du kannst eine Gruppe eigens dafür anlegen, eine Zugriffsfreigabe zu halten.
</Tip>

### Sprechen statt tippen

Willst du deinen Prompt nicht tippen? Tippe auf das Mikrofon-Symbol in einem beliebigen Genie-Chat-Composer, oder neben dem **Loslegen**-Button auf dem Workspace-Dashboard, um stattdessen zu diktieren. Deine Worte erscheinen im Composer, während du sprichst, statt erst auf einmal, wenn du aufhörst, und die Diktierfunktion erkennt Genie- und produktspezifische Begriffe — wie „Genie Cloud" oder „MCP" — statt sie misszuverstehen. Das Mikrofon bleibt auch verfügbar, nachdem du zu tippen begonnen hast — und sogar während eine Antwort noch eintrifft oder eine ältere Unterhaltung noch lädt —, sodass du eine Korrektur oder Ergänzung zu einem Entwurf diktieren kannst, ohne das Feld vorher zu leeren; tippe erneut darauf, um die Aufnahme zu beenden, oder sie stoppt nach fünf Minuten automatisch. Genie transkribiert das Gesagte, sodass du es vor dem Senden prüfen oder bearbeiten kannst — nichts wird automatisch abgeschickt. Spracheingabe ist im Workspace-Chat, im Chat-Panel eines Projekts, auf dem Workspace-Dashboard, in den eigenen Formularfeldern eines [Genie Tools](/de/workspaces/genie-tools) und in der Befehlspalette unter **Genie fragen** verfügbar, auf jedem Browser, der Mikrofon-Aufnahmen unterstützt.

### Workspace-Chats sind standardmäßig privat

Eine Unterhaltung mit der Workspace-KI ist nur für die Person sichtbar, die sie begonnen hat — niemand sonst im Workspace, auch keine Admins, kann sie standardmäßig öffnen oder darin schreiben. Teile sie über das Einstellungsmenü des Chats, um andere einzubeziehen — entweder mit **Alle im Workspace** oder mit **Nur ausgewählte Personen**, ausgewählt über eine Mitgliedersuche. In einem geteilten Chat zeigt jede Nachricht, wer sie geschrieben hat — Name und Avatar neben den jeweiligen Beiträgen —, sodass eine Unterhaltung mit mehreren Autor:innen lesbar bleibt. Deine **Letzten Chats** zeigen weiterhin standardmäßig nur deine eigenen Unterhaltungen, unabhängig davon, ob du einige davon geteilt hast, sodass deine Liste dadurch nicht mit fremden Threads überflutet wird. Falls du beim Support von Genie Hilfe zu einer Unterhaltung brauchst, kann dieser darauf zugreifen, um dir zu helfen, auch wenn sie nicht geteilt wurde.

Zieh einen Chat aus der Seitenleiste auf die **Angepinnt**-Ablagezone oben in der Liste, um ihn anzupinnen — zieh einen angepinnten Chat erneut dorthin, um ihn wieder zu lösen — oder nutze **Chat anpinnen** im selben Einstellungsmenü. Angepinnte Chats führen die Liste unter einer eigenen Überschrift an, vor den üblichen Datumsgruppen, überall, wo deine Chats aufgelistet werden.
...
<CardGroup cols={2}>
  ...
  <Card title="Tracing" icon="activity" href="/de/workspaces/tracing">
