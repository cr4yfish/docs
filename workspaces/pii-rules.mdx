...
Click the account row at the bottom of the workspace sidebar, choose **Workspace settings**, then open **PII Detection**.
...
<Tabs>
  ...
  <Tab title="Block">
    The message is rejected before it reaches the AI. The person sees a plain "Message held back" notice — not an error — naming which category of data triggered the block (for example, "Your message contained a credit card number"), with no retry button: sending the identical content would just block it again.

    Blocked messages are also logged (censored) so admins can review them, and anyone who can already see this log can turn on **email alerts** for new blocks — see [Activity log](/workspaces/overview#activity-log).
