...
Owners and admins always have full access. Anyone else needs the **See Governance** permission, granted on a custom role (or on the built-in Member role, workspace-wide) from the [Roles page](/workspaces/members#custom-roles) — the same way you'd grant any other role permission. It's a view-only grant: reviews and rule results, not rules or settings. Everyone else can't open Governance.

<Note>
Governance access used to be a separate per-member toggle on the settings page. That toggle is gone — if you'd granted someone view access that way before, grant it again as a role.
</Note>

<Note>
Owners and admins can also choose to **hide private projects** from the Governance view, from the workspace settings page.
</Note>
