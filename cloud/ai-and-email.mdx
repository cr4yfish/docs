...
Requests made through the built-in credentials are counted against your project and included in your Genie usage. See [Credits](/billing/credits) and [Plans](/billing/plans) for how usage maps to your plan. If you bring your own key (below), those requests are billed by the provider directly to you and do not count against your Genie usage.

To keep things fair for everyone, the built-in relay applies a **per-project rate limit** (a cap on requests per minute). Normal app traffic stays well under it; if you expect heavy, sustained volume, bring your own key or contact support to raise the limit.
