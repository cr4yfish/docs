...
At the bottom of the Cloud tab, the **Advanced** section contains a **Re-run bootstrap** button. Use this to upgrade an existing project to the latest Genie Cloud version, or to re-apply your configuration after making changes outside the settings UI. The bootstrap is fully safe to run at any time — it does not delete any data.

<Tip>
  Onboarding and private mode settings saved from the **Onboarding** tab now sync to your live Cloud instance automatically. You do not need to click **Re-run bootstrap** after changing those settings.
</Tip>
