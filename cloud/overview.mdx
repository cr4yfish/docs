...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding & access control" icon="shield-check">
    A welcome screen for new sign-ups and an optional private mode that restricts your app to approved users only. Configured from the Onboarding tab in project settings. See [App onboarding and access control](/cloud/onboarding).
  </Card>
</CardGroup>
...
At the bottom of the Cloud tab, the **Advanced** section contains a **Re-run bootstrap** button. Use this to upgrade an existing project to the latest Genie Cloud version, or to force a full re-sync of your Cloud configuration. The bootstrap is fully safe to run at any time — it does not delete any data.

<Tip>
  Onboarding and private mode settings saved in the **Onboarding** tab are applied to your Cloud instance automatically — no manual bootstrap needed. See [App onboarding and access control](/cloud/onboarding) to learn about these features.
</Tip>
