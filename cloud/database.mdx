...
## Reliable delete and update operations

When your app deletes or updates a record, the generated code checks whether the operation actually changed any rows before treating it as a success. If nothing was affected — for example, because the record no longer exists or because a security policy silently blocked the change — your app shows an informative error to the user instead of falsely reporting success.

This verification is built into Genie Cloud's generated code automatically. You do not need to add any checks yourself.
